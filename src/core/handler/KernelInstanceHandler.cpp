...
#include "core/kernel/PolyCoreStatsPoller.hpp"
...
#define isConnected (vCoreInstance->IsKernelRunning() || !activeKernels.empty())
    KernelInstanceHandler::KernelInstanceHandler(QObject *parent) : QObject(parent)
    {
        ...
        // Initialize PolyCore manager lazily
        polyCoreManager = std::make_unique<PolyCore::core::kernel::PolyCoreKernelManager>(this);
        polyCoreManager->AutoDetectKernels();
        // Wire PolyCore signals to existing handler signals/slots
        connect(polyCoreManager.get(), &PolyCore::core::kernel::PolyCoreKernelManager::OnKernelLogAvailable,
                this, &KernelInstanceHandler::OnV2RayKernelLog_p);
        connect(polyCoreManager.get(), &PolyCore::core::kernel::PolyCoreKernelManager::OnStatsDataAvailable,
                this, &KernelInstanceHandler::OnV2RayStatsDataRcvd_p);
        connect(polyCoreManager.get(), &PolyCore::core::kernel::PolyCoreKernelManager::OnConnectionStarted, this,
                [this](const ConnectionGroupPair &id, PolyCore::core::kernel::KernelType kernelType) {
                    currentId = id;
                    emit OnConnected(id);
                    // Start polling stats via Clash-compatible API when applicable
                    static PolyCore::core::kernel::PolyCoreStatsPoller poller;
                    if (GlobalConfig.kernelConfig.enableAPI && GlobalConfig.kernelConfig.useClashAPIStats)
                    {
                        QUrl apiUrl(QStringLiteral("http://%1:%2").arg(GlobalConfig.kernelConfig.apiHost)
                                        .arg(GlobalConfig.kernelConfig.statsPort));
                        poller.Start(apiUrl, 1000);
                        connect(&poller, &PolyCore::core::kernel::PolyCoreStatsPoller::OnStats, this,
                                &KernelInstanceHandler::OnV2RayStatsDataRcvd_p, Qt::UniqueConnection);
                    }
                });
        connect(polyCoreManager.get(), &PolyCore::core::kernel::PolyCoreKernelManager::OnConnectionStopped, this,
                [this](const ConnectionGroupPair &id) {
                    emit OnDisconnected(id);
                });
        connect(polyCoreManager.get(), &PolyCore::core::kernel::PolyCoreKernelManager::OnConnectionError, this,
                [this](const ConnectionGroupPair &id, const QString &err) {
                    emit OnCrashed(id, err);
                });
        //
    ...
    std::optional<QString> KernelInstanceHandler::StartConnection(const ConnectionGroupPair &id, CONFIGROOT fullConfig)
    {
        ...
        //}
        //
        if (GlobalConfig.kernelConfig.usePolyCoreManager)
        {
            // Use PolyCore multi-kernel manager
            using namespace PolyCore::core::kernel;
            // Apply preferred kernel from config
            // Per-connection override if provided
            const auto overrideKernel = connections[identifier.connectionId].preferredKernel;
            const auto effectivePref = overrideKernel.isEmpty() ? GlobalConfig.kernelConfig.preferredKernel : overrideKernel;
            polyCoreManager->SetPreferredKernelByName(effectivePref);
            // Decide a kernel based on preference and availability
            const auto preferred = PolyCoreKernelManager::KernelTypeFromString(effectivePref);
            auto err = polyCoreManager->StartConnection(id, fullConfig, preferred);
            if (err.has_value())
            {
                StopConnection();
                return err;
        ...
        else if (GlobalConfig.pluginConfig.v2rayIntegration)
        {
            // Process outbounds.
