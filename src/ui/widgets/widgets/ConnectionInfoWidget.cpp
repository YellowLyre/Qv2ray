...
void ConnectionInfoWidget::ShowDetails(const ConnectionGroupPair &_identifier)
{
    ...
    if (isConnection)
    {
        ...
        // Effective kernel label: per-connection override > global preferred (when PolyCore enabled)
        {
            QString effKernel = GlobalConfig.kernelConfig.preferredKernel;
            const auto meta = ConnectionManager->GetConnectionMetaObject(connectionId);
            if (!meta.preferredKernel.isEmpty()) effKernel = meta.preferredKernel;
            effectiveKernelLabel->setText(GlobalConfig.kernelConfig.usePolyCoreManager ? effKernel : QString("(legacy V2Ray path)"));
        }
        //
