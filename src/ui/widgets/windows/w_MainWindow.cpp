...
void MainWindow::OnEditRequested(const ConnectionId &id)
{
    ...
    else
    {
        ...
        // Load per-connection preferred kernel into dialog's combo if available
        if (auto combo = w.findChild<QComboBox*>("connectionPreferredKernelCombo"))
        {
            const auto meta = ConnectionManager->GetConnectionMetaObject(id);
            const QString pref = meta.preferredKernel.isEmpty() ? "(inherit global)" : meta.preferredKernel;
            int idx = combo->findText(pref, Qt::MatchFixedString);
            if (idx < 0) idx = 0;
            combo->setCurrentIndex(idx);
        }
        auto outboundEntry = w.OpenEditor();
        isChanged = w.result() == QDialog::Accepted;
        // Save per-connection preferred kernel selected in dialog
        if (isChanged)
        {
            if (auto combo = w.findChild<QComboBox*>("connectionPreferredKernelCombo"))
            {
                ...
                ConnectionManager->SetConnectionPreferredKernel(id, value);
            }
        }
