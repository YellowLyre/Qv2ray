...
PreferencesWindow::PreferencesWindow(QWidget *parent) : QvDialog("PreferenceWindow", parent), CurrentConfig()
{
    ...
    // Kernel Settings
    {
        ...
        apiHostTxt->setText(CurrentConfig.kernelConfig.apiHost);
        useClashAPIStatsCB->setChecked(CurrentConfig.kernelConfig.useClashAPIStats);
        // Preferred kernel
        {
            const auto pref = CurrentConfig.kernelConfig.preferredKernel;
            int idx = preferredKernelCombo->findText(pref, Qt::MatchFixedString);
            if (idx < 0)
            {
                idx = preferredKernelCombo->findText("sing-box", Qt::MatchFixedString);
            }
            if (idx >= 0)
                preferredKernelCombo->setCurrentIndex(idx);
            usePolyCoreCB->setChecked(CurrentConfig.kernelConfig.usePolyCoreManager);
        }
        //
    ...
    // Fill detected kernels table (read-only view)
    {
        using namespace PolyCore::core::kernel;
        PolyCoreKernelManager km;
        km.AutoDetectKernels();
        const auto kernels = km.GetAvailableKernels();
        kernelListTable->setColumnCount(8);
        kernelListTable->setRowCount(kernels.size());
        QStringList headers{ tr("Name"), tr("Path"), tr("Version"), tr("Format"), tr("Reality"), tr("Anylts"), tr("Protocols"), tr("Exec") };
        kernelListTable->setHorizontalHeaderLabels(headers);
        if (auto *hitemR = kernelListTable->horizontalHeaderItem(4))
        {
            hitemR->setToolTip(tr("Reality support: ✓ supported, ✗ not supported"));
            hitemR->setTextAlignment(Qt::AlignCenter);
        }
        if (auto *hitemA = kernelListTable->horizontalHeaderItem(5))
        {
            hitemA->setToolTip(tr("Anylts support: ✓ supported, ✗ not supported"));
            hitemA->setTextAlignment(Qt::AlignCenter);
        }
        int r = 0;
        for (const auto &k : kernels)
        {
            kernelListTable->setItem(r, 0, new QTableWidgetItem(k.name));
            kernelListTable->setItem(r, 1, new QTableWidgetItem(k.executablePath));
            kernelListTable->setItem(r, 2, new QTableWidgetItem(k.version));
            kernelListTable->setItem(r, 3, new QTableWidgetItem(k.configFormat));
            auto realityItem = new QTableWidgetItem(k.supportsReality ? "✓" : "✗");
            realityItem->setTextAlignment(Qt::AlignCenter);
            realityItem->setToolTip(k.supportsReality ? tr("Supported") : tr("Not supported"));
            if (k.supportsReality) realityItem->setForeground(QBrush(QColor(0, 128, 0))); else realityItem->setForeground(QBrush(QColor(176, 0, 32)));
            kernelListTable->setItem(r, 4, realityItem);
            auto anyltsItem = new QTableWidgetItem(k.supportsAnylts ? "✓" : "✗");
            anyltsItem->setTextAlignment(Qt::AlignCenter);
            anyltsItem->setToolTip(k.supportsAnylts ? tr("Supported") : tr("Not supported"));
            if (k.supportsAnylts) anyltsItem->setForeground(QBrush(QColor(0, 128, 0))); else anyltsItem->setForeground(QBrush(QColor(176, 0, 32)));
            kernelListTable->setItem(r, 5, anyltsItem);
            kernelListTable->setItem(r, 6, new QTableWidgetItem(k.supportedProtocols.join(", ")));
            // Exec status
            bool exec = QFileInfo(k.executablePath).isExecutable();
            auto execItem = new QTableWidgetItem(exec ? "✓" : "✗");
            execItem->setTextAlignment(Qt::AlignCenter);
            execItem->setToolTip(exec ? tr("Executable") : tr("Not executable or missing"));
            execItem->setForeground(exec ? QBrush(QColor(0,128,0)) : QBrush(QColor(176,0,32)));
            kernelListTable->setItem(r, 7, execItem);
            ++r;
        }
        kernelListTable->resizeColumnsToContents();
    }
    // Connection Settings
    {
...
void PreferencesWindow::on_useClashAPIStatsCB_stateChanged(int arg1)
{
    NEEDRESTART
    CurrentConfig.kernelConfig.useClashAPIStats = arg1 == Qt::Checked;
}

void PreferencesWindow::on_apiHostTxt_textEdited(const QString &arg1)
{
    NEEDRESTART
    CurrentConfig.kernelConfig.apiHost = arg1.trimmed();
}

void PreferencesWindow::on_testApiBtn_clicked()
{
    const auto host = apiHostTxt->text().trimmed();
    const auto port = statsPortBox->value();
    QUrl url(QStringLiteral("http://%1:%2/connections").arg(host).arg(port));
    QNetworkAccessManager nm; QNetworkRequest req(url);
    QEventLoop loop; QTimer to; to.setSingleShot(true); to.setInterval(3000);
    ...
    if (!reply->isFinished())
    {
        ...
        apiStatusLabel->setText(tr("Timeout"));
        apiStatusLabel->setStyleSheet("color: #b00020");
        return;
    }
    const auto statusVar = reply->attribute(QNetworkRequest::HttpStatusCodeAttribute);
    const int status = statusVar.isValid() ? statusVar.toInt() : -1;
    const bool ok = (reply->error() == QNetworkReply::NoError) && status >= 200 && status < 400;
    if (ok)
    {
        apiStatusLabel->setText(tr("OK (%1)").arg(status));
        apiStatusLabel->setStyleSheet("color: #018786");
    }
    else
    {
        apiStatusLabel->setText(tr("Error (%1)").arg(status));
        apiStatusLabel->setStyleSheet("color: #b00020");
        apiStatusLabel->setToolTip(reply->errorString());
    }
    reply->deleteLater();
}
...
void PreferencesWindow::on_preferredKernelCombo_currentIndexChanged(int)
{
    LOADINGCHECK
    NEEDRESTART
    CurrentConfig.kernelConfig.preferredKernel = preferredKernelCombo->currentText();
}

void PreferencesWindow::on_usePolyCoreCB_stateChanged(int arg1)
{
    LOADINGCHECK
    NEEDRESTART
    CurrentConfig.kernelConfig.usePolyCoreManager = arg1 == Qt::Checked;
}

void PreferencesWindow::on_kernelRescanBtn_clicked()
{
    using namespace PolyCore::core::kernel;
    PolyCoreKernelManager km;
    km.AutoDetectKernels();
    const auto kernels = km.GetAvailableKernels();
    kernelListTable->clearContents();
    kernelListTable->setRowCount(kernels.size());
    int r = 0;
    for (const auto &k : kernels)
    {
...
void PreferencesWindow::on_kernelSetPreferredBtn_clicked()
{
    int row = kernelListTable->currentRow();
    if (row < 0)
    {
        QvMessageBoxWarn(this, tr("Kernel Preference"), tr("Please select a kernel from the table."));
        return;
    }
    const auto nameItem = kernelListTable->item(row, 0);
    if (!nameItem)
        return;
    const auto kernelName = nameItem->text();
    if (kernelName.isEmpty())
        return;
    // Update preferred kernel combo and config
    int idx = preferredKernelCombo->findText(kernelName, Qt::MatchFixedString);
    if (idx >= 0)
    {
        preferredKernelCombo->setCurrentIndex(idx);
        CurrentConfig.kernelConfig.preferredKernel = kernelName;
        NEEDRESTART
    }
}

void PreferencesWindow::on_kernelListTable_itemDoubleClicked(QTableWidgetItem *item)
{
    Q_UNUSED(item)
    on_kernelSetPreferredBtn_clicked();
}

void PreferencesWindow::on_testKernelBtn_clicked()
{
    int row = kernelListTable->currentRow();
    if (row < 0)
    {
        QvMessageBoxWarn(this, tr("Kernel Test"), tr("Please select a kernel from the table."));
        return;
    }
    const auto nameItem = kernelListTable->item(row, 0);
    const auto pathItem = kernelListTable->item(row, 1);
    if (!nameItem || !pathItem)
        return;
    const auto kernelPath = pathItem->text();
    if (kernelPath.isEmpty())
        return;
    // Run '<exe> version' or '-v' for mihomo
    QStringList args;
    const auto name = nameItem->text().toLower();
    if (name.contains("mihomo") || name.contains("clash")) args << "-v"; else args << "version";
    QProcess proc; proc.start(kernelPath, args);
    if (!proc.waitForFinished(3000))
    {
        QvMessageBoxWarn(this, tr("Kernel Test"), tr("Timeout executing kernel."));
        return;
    }
    const auto out = QString::fromUtf8(proc.readAllStandardOutput() + proc.readAllStandardError());
    QvMessageBoxInfo(this, tr("Kernel Test Result"), out.isEmpty() ? tr("No output") : out);
}

void PreferencesWindow::on_refreshKernelVersionBtn_clicked()
{
    int row = kernelListTable->currentRow();
    if (row < 0)
        return;
    ...
    if (!proc.waitForFinished(3000))
        return;
    const auto out = QString::fromUtf8(proc.readAllStandardOutput() + proc.readAllStandardError());
    auto ktype = PolyCore::core::kernel::PolyCoreKernelManager::KernelTypeFromString(nameItem->text());
    const auto ver = PolyCore::core::kernel::PolyCoreKernelManager::ParseVersionFromOutput(ktype, out);
    kernelListTable->setItem(row, 2, new QTableWidgetItem(ver));
}
