...
OutboundEditor::OutboundEditor(const OUTBOUND &outboundEntry, QWidget *parent) : OutboundEditor(parent)
{
    ...
    reloadGUI();
    // Load per-connection preferred kernel if current selection context available
    // Try to read from last selected in MainWindow if possible is not trivial here; leave for caller
}
...
void OutboundEditor::on_buttonBox_accepted()
{
    ...
    // Persist per-connection preferred kernel if present in UI context
    // This dialog does not know ConnectionId; saving is handled by caller if needed.
}
