...
PolyCoreApplicationInterface::PolyCoreApplicationInterface()
{
    ConfigObject = new PolyCoreConfigObject;
    QvCoreApplication = this;
    LOG("PolyCore", QV2RAY_VERSION_STRING, "on", QSysInfo::prettyProductName(), QSysInfo::currentCpuArchitecture());
    DEBUG("PolyCore Start Time: ", QTime::currentTime().msecsSinceStartOfDay());
    ...
    licenseList << "Copyright (c) 2019-2021 Qv2ray Development Group (PolyCore fork)";
...
QStringList PolyCoreApplicationInterface::GetAssetsPaths(const QString &dirName) const
{
    // Configuration Path
    QStringList list;
