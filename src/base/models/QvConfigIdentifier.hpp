...
#define DECL_IDTYPE(type)                                                                                                                            \
    ...
    struct ConnectionObject : __Qv2rayConfigObjectBase
    {
        ...
        // Optional: per-connection preferred kernel override ("", "sing-box", "mihomo", "xray", "v2ray")
        QString preferredKernel;
        //
        int __qvConnectionRefCount = 0;
        JSONSTRUCT_COMPARE(ConnectionObject, lastConnected, latency, importSource, stats, preferredKernel, displayName, creationDate, lastUpdatedDate)
        JSONSTRUCT_REGISTER(ConnectionObject, F(lastConnected, latency, importSource, stats, preferredKernel), B(__Qv2rayConfigObjectBase))
    };
